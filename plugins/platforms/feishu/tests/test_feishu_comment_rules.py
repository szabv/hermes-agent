...
from hermes_agent_feishu.feishu_comment_rules import (
    CommentsConfig,
    CommentDocumentRule,
    ResolvedCommentRule,
    _MtimeCache,
...
class TestIsUserAllowed(unittest.TestCase):
    ...
    def test_pairing_checks_store(self):
        ...
        with patch(
            "hermes_agent_feishu.feishu_comment_rules._load_pairing_approved",
...
class TestLoadConfig(unittest.TestCase):
    def test_load_with_documents(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            ...
            path = Path(f.name)
        try:
            with patch("hermes_agent_feishu.feishu_comment_rules.RULES_FILE", path):
                with patch("hermes_agent_feishu.feishu_comment_rules._rules_cache", _MtimeCache(path)):
                    cfg = load_config()
    ...
    def test_load_missing_file_returns_defaults(self):
        with patch("hermes_agent_feishu.feishu_comment_rules._rules_cache", _MtimeCache(Path("/nonexistent"))):
            cfg = load_config()
        self.assertTrue(cfg.enabled)
...
class TestPairingStore(unittest.TestCase):
    def setUp(self):
        ...
        self._patcher_cache = patch(
            "hermes_agent_feishu.feishu_comment_rules._pairing_cache",
